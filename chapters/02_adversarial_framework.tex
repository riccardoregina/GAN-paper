...
Analogamente, $D$ vuole massimizzare la sua log-likelihood $\Lambda(D)$ e ovviamente anche la media di quest'ultima, ovvero: $$\frac{1}{n}\sum_{i=1}^{m}{D(x^{(i)})} + \frac{1}{n}\sum_{i=1}^{m}{(1-D(G({z^{(i)})}))}$$
