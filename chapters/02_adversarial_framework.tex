...
Analogamente, $D$ vuole massimizzare la sua log-likelihood $\Lambda(D)$ e ovviamente anche la media di quest'ultima, ovvero: $$\frac{1}{m}\sum_{i=1}^{m}{D(x^{(i)})} + \frac{1}{m}\sum_{i=1}^{m}{(1-D(G({z^{(i)})}))}$$
...
In altre parole, $D$ e $G$ fanno parte del seguente gioco minimax: $$\min_{G}\max_{D}V(D,G)=\bb{E}_{x\approx p_{data}}[D(x)] + \bb{E}_{z \approx p_{g}}[{1-D(G({z}))}]$$
